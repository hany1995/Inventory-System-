package GUI;

import Core.Product;
import Core.Users.Cashier;
import Core.Users.InventoryClerk;
import Core.Users.Manager;
import Core.Users.Supervisor;
import java.awt.*;
import java.awt.event.*;
import javax.swing.*;

public class SearchProduct extends Parent implements ActionListener {

    public SearchProduct() {
        setLocation(100, 200);
        setSize(400, 300);
        this.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
        this.setTitle("\t\t(-->( Welcome To You In Inventory )<--)");
        this.setLocationRelativeTo(null);
        this.setResizable(false);
        this.getContentPane().setLayout(null);

        Label1 = new JLabel("Search an a Product");
        add(Label1);
        this.Label2 = new JLabel("Enter an Parcode");
        add(Label2);
        this.Text1 = new JTextField("");
        add(Text1);
        area1 = new JTextArea("");
        add(area1);
        area1.setEnabled(false);

        this.Next = new JButton("Search");
        add(Next);
        this.Back = new JButton("Back");
        add(Back);

        Label1.setBounds(10, 10, 300, 30);
        Label2.setBounds(50, 50, 150, 40);
        Text1.setBounds(160, 50, 150, 40);
        area1.setBounds(10, 100, 300, 100);

        Next.setBounds(220, 230, 150, 40);
        Back.setBounds(10, 230, 150, 40);
        Next.addActionListener(this);
        Back.addActionListener(this);
    }

    public static void main(String[] args) {
        SearchProduct main = new SearchProduct();

        main.setVisible(true);
    }

    @Override
    public void actionPerformed(ActionEvent e) {

        if (e.getSource() == Next) {
            Supervisor us = new Supervisor();
            Product ch = new Product();
            ch.ParCode = Text1.getText();
            area1.setText(us.searchProduct(ch.ParCode));
        } else if (e.getSource() == Back) {
            dispose();
            new Supervisor_GUI().setVisible(true);
        }

    }
}
